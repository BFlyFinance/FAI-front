<template>
  <div
    v-if="accountHash"
    class="btn primary account-btn"
    :data-clipboard-text="accountHash.toString()"
  >
    {{ btnText }}
    ...
    setup() {
      ...
      const handleAccountChanged = (accounts = []) => {
        if (accounts.length > 0) {
          ...
          const clipboard = new window.ClipboardJS('.account-btn');
          clipboard.on('success', () => {
            $message.success(t('borrow.msg.copySuccess'));
          });
